...
@frappe.whitelist()
def getDateMonthDiff( invoice_date, start_date, end_date, month_factor):
    month_count = 0
    no_month = 0
    month_float = 0
    # frappe.msgprint("start_date: " + str(start_date) + "  --- end_date: " + str(end_date))
    while invoice_date <= end_date:
        # period_end_date = add_days(add_months(invoice_date, month_factor), -1)

        month_count = month_count + month_factor
        invoice_date = add_months(invoice_date, month_factor)

        # if invoice_date.month == start_date.month and  invoice_date.year == start_date.year:
        #     days = float(
        #         date_diff(getdate(invoice_date), getdate(add_months(start_date, no_month)))
        #         + 1
        #     )
        ...
        #     start_date = add_months(invoice_date, month_factor)
        #     gdmd_invoice  = f"`gdmd_invoice: {invoice_date} - days" {days} -`"
        #     frappe.msgprint(gdmd_invoice)
        # # elif start_date <= end_date or invoice_date.month != start_date.month:
        # else:
